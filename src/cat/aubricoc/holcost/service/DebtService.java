...
public class DebtService {
	...
	public Debt getDudeDebt(Dude dude) {
		...
		List<Cost> payedCosts = costDao.getByPayer(dude.getId());
		for (Cost cost : payedCosts) {
			payedAmount += cost.getAmount();
		}
		
		List<DudeCost> dudeCosts = dudeCostDao.getByDude(dude.getId());
		
		for (DudeCost dudeCost : dudeCosts) {
			Cost cost = costDao.getById(dudeCost.getCost());
			List<DudeCost> participants = dudeCostDao.getByCost(cost.getId());
